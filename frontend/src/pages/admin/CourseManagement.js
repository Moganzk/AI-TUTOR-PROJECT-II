...
const CourseManagement = () => {
      ...
      const sampleCourses = [
        {
          ...
          thumbnail_url: '/api/placeholder/40/40'
        },
        {
  ...
  // Fetch course statistics with timeout
  const fetchStats = async () => {
    try {
      ...
      setStats({
        ...
        inactive_courses: courses.filter(c => !c.is_active).length,
        total_enrollments: courses.reduce((sum, c) => sum + (c.current_students || 0), 0),
        avg_enrollment_per_course: courses.length > 0 ? 
          Math.round(courses.reduce((sum, c) => sum + (c.current_students || 0), 0) / courses.length) : 0
      });
    }
  };

  // Fetch instructors for filter dropdown with timeout
  const fetchInstructors = async () => {
    try {
      ...
      // Set fallback instructors
      setInstructors([
        { id: 'user-staff-001', name: 'Staff Morgan', email: 'morganstyles50@gmail.com' },
        { id: '3dba0721-b5ea-46a3-aaaf-cc19f210d72e', name: 'Admin Alice', email: 'alicemwapo24@gmail.com' }
      ]);
    }
  };
